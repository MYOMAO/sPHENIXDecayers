...
SvtxEvaluator::SvtxEvaluator(const string& /*name*/, const string& filename, const string& trackmapname,
  ...
  , _timer(nullptr)
{
	std::cout << "FUCKIN SVTX BROI ORB" << std::endl;
}
...
void SvtxEvaluator::fillOutputNtuples(PHCompositeNode* topNode)
{
  ...
  if (_ntp_track)
  {
	std::cout << "SUCK SVTX EVAL - Pass 0" << 	std::endl;
    if (Verbosity() > 1)
    {
      cout << "Filling ntp_track " << endl;
      _timer->restart();
    }
	std::cout << "SUCK SVTX EVAL - Pass 1" << 	std::endl;
	...
	if (trackmap)
    {
  	  std::cout << "SUCK SVTX EVAL - Pass 2" << 	std::endl;
		
      for (SvtxTrackMap::Iter iter = trackmap->begin();
           iter != trackmap->end();
           ++iter)
      {
	  	std::cout << "SUCK SVTX EVAL - Pass 3" << 	std::endl;
        SvtxTrack* track = iter->second;
